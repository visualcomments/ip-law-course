...
class LegalChangeHeuristic:
    ...
    Для каждой нормы накапливаются фактические события изменения (поправки) и
    рёбра влияния (отсылки/влечёт_изменение). Скоринг использует:
      память недавних поправок (recency-память), повторяемость (норма часто меняется),
      ...
      давность последней поправки (для базовых признаков).
    """
